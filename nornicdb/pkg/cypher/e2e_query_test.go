...
//
// These tests cover every type of Cypher query syntax supported by the ASTExecutor.
package cypher
...
func setupTestExecutor(t *testing.T) (*ASTExecutor, context.Context) {
...
func setupTestData(t *testing.T, exec *ASTExecutor, ctx context.Context) {
	// Create test nodes
	queries := []string{
...
func TestE2E_Match_AllNodes(t *testing.T) {
	exec, ctx := setupTestExecutor(t)
	setupTestData(t, exec, ctx)
